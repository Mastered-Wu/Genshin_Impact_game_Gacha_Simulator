...
第一版不包含用户账号、支付、实时游戏数据抓取、官方美术、官方 Logo 或保存玩家模拟状态的远程后端。网页由 Cloudflare Pages 静态托管；公网模式下，C++ 规则引擎以 WebAssembly 在访客浏览器内运行。
...
前端使用纯 HTML/CSS/JavaScript。桌面本地运行时通过 `fetch()` 调用 C++ HTTP API；静态公网部署时调用同一 C++ 规则层编译出的 WebAssembly 适配接口，不得在 JavaScript 中另行实现抽卡规则。界面需要兼容桌面和移动端，静态资源路径使用相对地址，兼容 Cloudflare Pages 提供的公开网址和自定义域名。由于项目名称和使用语境是中文，UI 标签可以使用中文。

## 数据与持久化

卡池定义存放在 `configs/banners.json`。第一版运行状态可以只保存在内存中：本地服务器重启后重置；静态公网模式中，每个浏览器页面会话各自创建独立 C++ 规则引擎实例，互不共享状态，刷新页面后重置。UID 只用于本地页面入口校验，不发送到服务器，也不是账号认证。
...
- GitHub Actions 使用 Emscripten 将 C++ 规则层和状态控制器编译为 WebAssembly，并通过 Wrangler 上传到 Cloudflare Pages。
- Cloudflare Pages 仅托管前端静态文件和生成的 WASM 文件，不提供共享状态的远程 API 服务。
- 配置 Cloudflare API token、Account ID 和 Pages 项目名后，推送到 `main` 会自动构建和部署。
- 公网运行状态彼此隔离且只保存在各访客当前页面会话中，刷新即重置。
