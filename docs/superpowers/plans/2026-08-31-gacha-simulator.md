...
- 保底和保底状态行。
- 顶部资源区显示可编辑星石、只读星轨之缘和只读恒辉之缘，并支持按 160 星石兑换 1 个当前卡池对应缘券。
- 主抽卡面板不展示保底计数器或命定值计数器，保留空间用于后续卡池背景图。
- `单抽` 和 `十连` 按钮。
- 抽卡结果弹窗。
- 独立的统计页和历史页。
...
- 默认加载角色卡池。
- 输入资源数量后显示当前资源和可抽次数。
- 单抽完成后弹出结果窗口并展示一条结果，同时新增一条历史记录。
- 十连完成后弹出结果窗口并展示十条结果。
- 武器定轨选择器只在武器卡池显示。
- 进入 UID 后状态为默认值，点击重置会清空计数器和历史；主界面不显示保底计数，用户可从历史自行计算。
